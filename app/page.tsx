...
export default function Home() {
  ...
  const [quantidades,setQuantidades] = useState<Record<string,number>>({
    'Pão Grande':0,'Pão c/ Cinco Fatias':0,'Rosca Tradicional':0,'Rosca Recheada (goiabada)':0,'Rosca Recheada (chocolate)':0,'Fatia Húngara':0
  })

  const total = Object.keys(quantidades).reduce((soma,item)=>soma + quantidades[item]*precos[item],0)

  function diminuir(item:string){ setQuantidades(prev=>({...prev,[item]:Math.max(0,prev[item]-1})) }
  function aumentar(item:string){ setQuantidades(prev=>({...prev,[item]:prev[item]+1})) }
  ...
  return (
    <motion.div className='min-h-screen bg-gradient-to-b from-yellow-50 to-yellow-100 dark:from-gray-900 dark:to-gray-800 flex flex-col items-center py-10 px-4 transition-colors'>
      <motion.h1 className='text-4xl font-extrabold mb-8 text-yellow-900 dark:text-yellow-400 drop-shadow-lg' initial={{opacity:0,y:-30}} animate={{opacity:1,y:0}} transition={{duration:0.7}}>🥖 Pães Caseiros</motion.h1>
      <div className='w-full max-w-md space-y-5'>
        {Object.keys(quantidades).map((item,idx)=>(
          <motion.div key={item} className='flex justify-between items-center p-4 rounded-3xl shadow-lg hover:shadow-2xl transition-transform transform hover:-translate-y-1 bg-gradient-to-r from-yellow-100 to-yellow-200 dark:from-gray-700 dark:to-gray-600' custom={idx} initial='hidden' animate='visible' variants={cardVariants}>
            <div className='flex items-center gap-3'>{icones[item]}<div><h2 className='font-semibold text-yellow-900 dark:text-yellow-300'>{item}</h2><p className='text-gray-700 dark:text-gray-300 font-medium'>R$ {precos[item].toFixed(2)}</p></div></div>
            <div className='flex items-center space-x-2'>
              ...
              <motion.span key={quantidades[item]} animate={{scale:[1,1.4,1]}} transition={{duration:0.35}} className='w-8 text-center font-bold text-lg text-yellow-900 dark:text-yellow-200'>{quantidades[item]}</motion.span>
      ...
      <motion.div className='mt-8 w-full max-w-md flex justify-between items-center' initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:0.6,duration:0.5}}>
        <motion.h2 className='text-2xl font-bold text-yellow-900 dark:text-yellow-200' animate={{scale:[1,1.05,1]}} transition={{repeat:Infinity,duration:1}}>Total: R$ {total.toFixed(2)}</motion.h2>
      ...
      <AnimatePresence>
        {showPix && pixCode && <motion.div className='mt-10 flex flex-col items-center p-8 rounded-3xl shadow-2xl bg-gradient-to-tr from-white to-yellow-50 dark:from-gray-800 dark:to-gray-700' initial={{opacity:0,scale:0.8}} animate={{opacity:1,scale:1}} exit={{opacity:0,scale:0.8}} transition={{duration:0.5}}>
